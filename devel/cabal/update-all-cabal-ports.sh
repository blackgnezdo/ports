...
typeset -i RUN_PACKAGE=0 GIT_COMMIT=0 CONTINUE=0
...
LAST_PORT=""
((CONTINUE)) && [[ -f "$PROGRESS_FILE" ]] && LAST_PORT=$(<"$PROGRESS_FILE")

typeset -i SUCCESS=0 SKIPPED=0 FAILED=0 CURRENT=0 SKIP_UNTIL=0
[[ -n "$LAST_PORT" ]] && SKIP_UNTIL=1

for port in $CABAL_PORTS; do
	((CURRENT++))

	if ((SKIP_UNTIL)); then
		[[ "$port" == "$LAST_PORT" ]] && SKIP_UNTIL=0 || continue
	fi

	print "==> [$CURRENT/$TOTAL] $port"
	print "$port" > "$PROGRESS_FILE"
	...
	if $UPDATE_CMD; then
		...
		if git diff --quiet Makefile cabal.inc 2>/dev/null; then
			...
			if ((GIT_COMMIT)); then
				...
				git commit -m "$port: update to $VERSION" || true
			fi
		fi
		cd - >/dev/null
	else
